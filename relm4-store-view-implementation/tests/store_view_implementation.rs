...
type Assertion<Window> = &'static dyn Fn(&Vec<TestRecord>, &StoreViewImplementation<TestConfig<Window>>, &Vec<TestRecord>) -> ();
type Prepare<Window> = &'static dyn Fn(&StoreViewImplementation<TestConfig<Window>>) -> bool;

struct StoreViewTest<Window>
where
