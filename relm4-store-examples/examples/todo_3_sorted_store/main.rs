...
fn main() -> Result<()> {
    log4rs::init_file("relm4-store-examples/examples/todo_3_sorted_store/etc/log4rs.yaml", Default::default()).unwrap();

    log::info!("");
    log::info!("Todo 3 example!");
    log::info!("");

    
    let app_id = "store.relm4.example.todo-3-sorted-store";
